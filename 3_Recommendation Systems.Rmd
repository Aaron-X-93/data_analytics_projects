...
ggplot() + 
  ...
  geom_histogram(data=train_2, aes(x=n_items_train),col="blue",alpha = 0.3) +
...
head(re_order)
re_order_sample <- samples %>%  
            group_by(reordered) %>% 
            summarize(count = n()) %>% 
  mutate(reordered = as.factor(reordered)) %>%
  mutate(proportion = count/sum(count))
head(re_order_sample)
```
...
tmp1_1 <- samples %>%
  ...
  top_n(n=20, wt=count) %>%  
  mutate(percentage=count/sum(count))
p1 = ggplot (tmp1_1, aes(x=reorder(product_name,count), y=percentage)) +  
  geom_col() + 
  ggtitle('Sample Products Top 20') + 
  ylab('Percentage of Orders') +
  theme (axis.text.x=element_text(angle=90, hjust=1, vjust=0.5),
         axis.title.x = element_blank()) 
p2 = ggplot (data = tmp1_1, aes( x= '', y=percentage )) + 
  ggtitle('Sample Products Top 20') + 
  ylab('percentage.of.orders') + 
  geom_boxplot() + 
  xlab('Products')
